"""
Level 4 Training Script

Train a DQN agent on Level 4 Ludo (2p2t with capturing).
Goal: Achieve 55%+ win rate against random opponent.

Key differences from Level 1:
- 16D state space (vs 4D)
- Multi-Token mechanics (offensive/defensive strategy)
- Safe zones (strategic positioning)
- Target: 55% win rate (vs 65% for Level 1)
"""
...
def train_level4(
    ...
    checkpoint_dir='checkpoints/level4',
    device='cpu',
    ...
    print("="*80)
    print(f"Target: 55%+ win rate in {num_episodes} episodes")
    print(f"New features: Multi-Token, Safe zones, 16D state space")
    print(f"\nHyperparameters:")
    print(f"  Learning rate: {learning_rate}")
    print(f"  Discount factor: {discount_factor}")
    print(f"  Epsilon: {epsilon_start} → {epsilon_min} (decay={epsilon_decay})")
    print(f"  Batch size: {batch_size}")
    print(f"  State dim: 16D (vs 4D in Level 1)")
    ...
    # Create environment
    env = Level4StochasticLudo(seed=seed)

    # Create DQN agent (16D state space)
    agent = SimpleDQNAgent(
        state_dim=16,  # Level 4 has 16D state
        action_dim=3,
    ...
    # (Note: Can't directly transfer since state dims differ: 4D → 16D)
    ...
    for episode in range(num_episodes):
        ...
        if (episode + 1) % eval_frequency == 0:
            ...
            if eval_stats['win_rate'] >= 0.60:
                tqdm.write(f"\n{'='*60}")
                tqdm.write(f"🎉 TARGET REACHED! Win rate: {eval_stats['win_rate']:.2%} >= 55%")
                tqdm.write(f"{'='*60}")
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_path = results_dir / f"level4_training_{timestamp}.json"
...
def main():
    parser = argparse.ArgumentParser(description="Train Level 4 agent")
