"""
Level 3 Training Script

Train a DQN agent on Level 3 Ludo (2p2t with capturing).
Goal: Achieve 58%+ win rate against random opponent.

Key differences from Level 1:
- 14D state space (vs 4D)
- Multi-Token mechanics (offensive/defensive strategy)
- Safe zones (strategic positioning)
- Target: 58% win rate (vs 65% for Level 1)
"""
...
def train_level3(
    ...
    checkpoint_dir='checkpoints/level3',
    device='cpu',
    load_level1_model=None,
):
    """Train Level 3 agent."""
    print("="*80)
    print("LEVEL 2 TRAINING: Ludo with Multi-Token (2p2t)")
    print("="*80)
    print(f"Target: 58%+ win rate in {num_episodes} episodes")
    print(f"New features: Multi-Token, Safe zones, 14D state space")
    print(f"\nHyperparameters:")
    print(f"  Learning rate: {learning_rate}")
    print(f"  Discount factor: {discount_factor}")
    print(f"  Epsilon: {epsilon_start} → {epsilon_min} (decay={epsilon_decay})")
    print(f"  Batch size: {batch_size}")
    print(f"  State dim: 14D (vs 4D in Level 1)")
    print(f"  Device: {device}")
    if load_level1_model:
        print(f"  Transfer learning: Loading Level 1 model")
    print("="*80)

    # Create environment
    env = Level3MultiTokenLudo(seed=seed)

    # Create DQN agent (14D state space)
    agent = SimpleDQNAgent(
        state_dim=14,  # Level 3 has 14D state
        action_dim=3,
    ...
    # Optional: Load Level 1 model for transfer learning
    # (Note: Can't directly transfer since state dims differ: 4D → 14D)
    # Would need to implement feature mapping or start fresh
    if load_level1_model:
        print("⚠️  Transfer learning from Level 1 not implemented (state dim mismatch)")
    ...
    for episode in range(num_episodes):
        ...
        if (episode + 1) % eval_frequency == 0:
            ...
            if eval_stats['win_rate'] >= 0.60:
                tqdm.write(f"\n{'='*60}")
                tqdm.write(f"🎉 TARGET REACHED! Win rate: {eval_stats['win_rate']:.2%} >= 58%")
                tqdm.write(f"{'='*60}")
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_path = results_dir / f"level3_training_{timestamp}.json"
...
def main():
    parser = argparse.ArgumentParser(description="Train Level 3 agent")
    ...
    parser.add_argument('--load_level1', type=str, default=None, help="Path to Level 1 model (not implemented)")
    ...
    train_level3(
        ...
        device=args.device,
        load_level1_model=args.load_level1,
    )
