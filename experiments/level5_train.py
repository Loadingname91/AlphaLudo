"""
Level 5 Training Script

Train a DQN agent on Level 5 Ludo (4 players, 2 tokens each).
Goal: Achieve 52%+ win rate against 3 random opponents.

Key differences from Level 4:
- 4 players (vs 2 players)
- Competing against 3 opponents simultaneously
- Target: 52% win rate (harder than Level 4's 55%)
- Much more complex strategic landscape

Lessons from Level 3-4:
- Use large replay buffer (300k) to prevent catastrophic forgetting
- Use slow epsilon decay (0.999) for more exploration
- Use low learning rate (5e-5) for stability
"""
...
def train_level5(
    ...
    checkpoint_dir='checkpoints/level5',
    device='cpu',
    load_level4_model=None,
):
    """Train Level 5 agent."""
    print("="*80)
    print("LEVEL 5 TRAINING: Multi-Agent Chaos (4 players, 2 tokens each)")
    print("="*80)
    print(f"Target: 52%+ win rate in {num_episodes} episodes")
    print(f"Challenge: Competing against 3 random opponents simultaneously")
    print(f"\nHyperparameters:")
    print(f"  Learning rate: {learning_rate}")
    print(f"  Discount factor: {discount_factor}")
    print(f"  Epsilon: {epsilon_start} → {epsilon_min} (decay={epsilon_decay})")
    print(f"  Batch size: {batch_size}")
    print(f"  Replay buffer: {replay_buffer_size} (3x Level 4)")
    print(f"  State dim: 16D (same as Level 4)")
    print(f"  Device: {device}")
    if load_level4_model:
        print(f"  Transfer learning: Loading Level 4 model")
    print("="*80)

    # Create environment
    env = Level5MultiAgentLudo(seed=seed)

    # Create DQN agent (16D state space)
    agent = SimpleDQNAgent(
        state_dim=16,  # Level 5 has 16D state (same as Level 4!)
        action_dim=3,
    ...
    # Optional: Load Level 4 model for transfer learning
    if load_level4_model:
        try:
            agent.load(load_level4_model)
            print(f"✓ Loaded Level 4 model: {load_level4_model}")
            print("  Continuing training from Level 4 weights...")
        except Exception as e:
            print(f"⚠️  Failed to load Level 4 model: {e}")
            print("   Starting fresh with random weights")
    ...
    for episode in range(num_episodes):
        ...
        if (episode + 1) % eval_frequency == 0:
            ...
            if eval_stats['win_rate'] >= 0.52:
                tqdm.write(f"\n{'='*60}")
                tqdm.write(f"🎉 TARGET REACHED! Win rate: {eval_stats['win_rate']:.2%} >= 52%")
                tqdm.write(f"{'='*60}")
    ...
    print(f"Captures by agent: {final_eval['avg_captures_by_agent']:.2f}/episode")
    print(f"Captures of agent: {final_eval['avg_captures_of_agent']:.2f}/episode")
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_path = results_dir / f"level5_training_{timestamp}.json"
...
def main():
    parser = argparse.ArgumentParser(description="Train Level 5 agent")
    ...
    parser.add_argument('--load_level4', type=str, default=None, help="Path to Level 4 model for transfer learning")
    ...
    train_level5(
        ...
        device=args.device,
        load_level4_model=args.load_level4,
    )
