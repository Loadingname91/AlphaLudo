"""
Level 2 Training Script

Train a DQN agent on Level 2 Ludo (2p1t with capturing).
Goal: Achieve 60%+ win rate against random opponent.

Key differences from Level 1:
- 8D state space (vs 4D)
- Capturing mechanics (offensive/defensive strategy)
- Safe zones (strategic positioning)
- Target: 60% win rate (vs 65% for Level 1)
"""
...
def train_level2(
    ...
    checkpoint_dir='checkpoints/level2',
    device='cpu',
    load_level1_model=None,
):
    """Train Level 2 agent."""
    print("="*80)
    print("LEVEL 2 TRAINING: Ludo with Capturing (2p1t)")
    print("="*80)
    print(f"Target: 60%+ win rate in {num_episodes} episodes")
    print(f"New features: Capturing, Safe zones, 8D state space")
    print(f"\nHyperparameters:")
    print(f"  Learning rate: {learning_rate}")
    print(f"  Discount factor: {discount_factor}")
    print(f"  Epsilon: {epsilon_start} → {epsilon_min} (decay={epsilon_decay})")
    print(f"  Batch size: {batch_size}")
    print(f"  State dim: 8D (vs 4D in Level 1)")
    ...
    # Create environment
    env = Level2InteractionLudo(seed=seed)

    # Create DQN agent (8D state space)
    agent = SimpleDQNAgent(
        state_dim=8,  # Level 2 has 8D state
        action_dim=2,
    ...
    # (Note: Can't directly transfer since state dims differ: 4D → 8D)
    ...
    for episode in range(num_episodes):
        ...
        if (episode + 1) % eval_frequency == 0:
            ...
            if eval_stats['win_rate'] >= 0.60:
                tqdm.write(f"\n{'='*60}")
                tqdm.write(f"🎉 TARGET REACHED! Win rate: {eval_stats['win_rate']:.2%} >= 60%")
                tqdm.write(f"{'='*60}")
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_path = results_dir / f"level2_training_{timestamp}.json"
...
def main():
    parser = argparse.ArgumentParser(description="Train Level 2 agent")
