"""
Level 4: Full Stochasticity (Six-to-Exit Rule)
...
class Level4StochasticLudo(gym.Env):
    """
    Level 3 Ludo with 2 tokens per player.

    Key challenge: Handle early-game stochasticity (need 6 to exit home) + token coordination - which token to move when?
    """
    ...
    def __init__(
        ...
        # Observation space: 16D (7 for my tokens + 7 for opponent tokens)
    ...
    def _can_move_token(self, tokens: List[int], token_idx: int, dice: int) -> bool:
        ...
        Level 4: Implements six-to-exit rule!
        - If token is at home (position 0), can only move if dice == 6
        - Otherwise, can move if not at goal
        """
    ...
    def _get_observation(self) -> np.ndarray:
        """
        Get current observation (16D vector).
        ...
        My tokens (8D):
          ...
          - can_exit_this_turn (1.0 if rolled 6 and have tokens home, else 0.0)

        Opponent tokens (8D): same structure
        """
        ...
        # Can exit this turn? (rolled 6 and have tokens at home)
        my_can_exit = 1.0 if (self.current_dice == 6 and my_num_home > 0) else 0.0
        ...
        # Opponent can exit (unknown dice, estimate 1/6 chance if tokens home)
        opp_can_exit = (1.0/6.0) if opp_num_home > 0 else 0.0
...
def make_level4_env(seed: Optional[int] = None, render_mode: Optional[str] = None):
    """Create a Level 3 multi-token Ludo environment."""
    return Level4StochasticLudo(render_mode=render_mode, seed=seed)
